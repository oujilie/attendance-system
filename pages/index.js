...
// 根據截圖 image_6ed43f.png 完整補齊的假別清單
const LEAVE_TYPES = [
  ...
  return (
    <div className="min-h-screen bg-[#F4F6F8] text-gray-800 font-sans p-4 relative pb-32 overflow-x-hidden">
          ...
          <div className="bg-white rounded-3xl w-full max-w-md shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 border border-gray-200">
            <div className="bg-gray-900 text-white p-6 flex justify-between items-center">
              <div>
                ...
                <p className="text-xs text-gray-400 font-bold uppercase tracking-tight">代號: {INITIAL_EMPLOYEES.find(e => e.id === editingPunch.empId)?.name} | {editingPunch.day}日</p>
              </div>
      ...
      <div className="flex flex-col items-center mb-10">
        {!isAdmin && (
          <div className="mb-4 flex flex-wrap justify-center gap-2 animate-in fade-in slide-in-from-top-2">
              ...
              {INITIAL_EMPLOYEES.map((emp) => (
                <tr key={emp.id} className={`border-b border-gray-100 transition-colors ${currentUser === emp.id ? 'bg-blue-50/30' : 'hover:bg-gray-50/30'}`}>
                  ...
                  {daysArray.map(day => {
                    ...
                    return (
                      <td key={day} onClick={() => handleCellClick(emp.id, day)} className={`border-r border-gray-100 text-center relative h-16 group/cell cursor-pointer hover:bg-gray-50/50 ${typeInfo ? typeInfo.bg : ''}`}>
      ...
      <div className="mt-8 flex justify-between items-center px-2">
        {isAdmin && (
          <button onClick={() => setShowHistory(!showHistory)} className="flex items-center space-x-3 px-6 py-4 bg-white border border-gray-200 rounded-2xl shadow-sm hover:border-red-400 hover:shadow-lg transition-all text-sm font-black text-gray-500 group">
      ...
      {/* 底部漂浮假別選擇器 - 恢復最初樣式並完整補齊按鈕 */}
      <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-[200] flex flex-wrap justify-center gap-2 max-w-[95vw] px-6 py-3 bg-white/80 backdrop-blur-md rounded-2xl shadow-2xl border border-gray-200 animate-in slide-in-from-bottom-10 duration-500 overflow-visible">
