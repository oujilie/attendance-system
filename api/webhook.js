// api/webhook.js
const https = require('https');

export default async function handler(req, res) {
    if (req.method !== 'POST') {
        return res.status(200).send('Webhook is active');
    }

    // 封裝成 Promise，強迫 Vercel 等待發送完畢
    const sendMessage = (data) => {
        return new Promise((resolve, reject) => {
            const options = {
                hostname: 'api.telegram.org',
                port: 443,
                path: `/bot8789257005:AAGi3w0zTl3K7jwpFlXPtvjxpBciWbUAg-s/sendMessage`,
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Content-Length': Buffer.byteLength(data)
                }
            };

            const tgReq = https.request(options, (tgRes) => {
                let body = '';
                tgRes.on('data', (chunk) => body += chunk);
                tgRes.on('end', () => resolve(body));
            });

            tgReq.on('error', (e) => reject(e));
            tgReq.write(data);
            tgReq.end();
        });
    };

    try {
        const { message } = req.body;
        if (!message || !message.text) return res.status(200).send('OK');

        const chatId = message.chat.id;
        const text = message.text.toLowerCase().trim();
        const now = new Date().toLocaleString('zh-TW', { timeZone: 'Asia/Taipei' });

        let responseText = "";
        let replyMarkup = null;

        // 指令判斷邏輯
        if (text.startsWith('/check')) {
            responseText = "📊 <b>精確工時模擬結算</b>\n━━━━━━━━━━━━━━\n👤 員工：001\n🕒 上班：07:55:00\n🕒 下班：17:30:15\n\n⚖️ <b>計算結果：</b>\n├ 一般工時：8.000 h\n└ 加班工時：1.504 h";
        } else if (text.startsWith('/today')) {
            responseText = "📅 <b>今日打卡總覽</b>\n━━━━━━━━━━━━━━\n🟢 已上班：4 人\n🔴 已下班：0 人";
            replyMarkup = { inline_keyboard: [[{ text: "🌐 開啟管理系統", url: "https://attendance-system-bice-one.vercel.app" }]] };
        } else if (text.startsWith('/status')) {
            responseText = `✅ 伺服器狀態：正常\n⏰ 時間：${now}`;
        } else if (text.startsWith('/leaves')) {
            responseText = "🏖 <b>今日請假人員</b>\n━━━━━━━━━━━━━━\n● 005 - 病假\n● 008 - 特休";
        } else {
            responseText = "👋 歡迎！您可以輸入以下指令：\n/status - 狀態\n/today - 概況\n/check - 工時\n/leaves - 請假";
        }

        const data = JSON.stringify({
            chat_id: chatId,
            text: responseText,
            parse_mode: 'HTML',
            reply_markup: replyMarkup
        });

        // 關鍵：使用 await 等待 Telegram 回應
        await sendMessage(data);
        
        return res.status(200).json({ status: 'success' });

    } catch (error) {
        console.error('Error:', error);
        return res.status(200).json({ error: error.message });
    }
}
